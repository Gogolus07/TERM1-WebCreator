...
void MainWindow::compilerLeSite(){
    ...
    QSaveFile file(saveDir+"/"+projectName+".html");//file(saveDir, ios::out | ios::trunc);

    if(m_site==nullptr){
        QMessageBox::information(this, "Attention", "Aucun projet ouvert");
        return;
    }
    while(saveDir == ""){
        saveDir = QFileDialog::getSaveFileName(this, "Enregistrer un fichier", QString(), "HTML (*.html)");
    }

    QFile siteHTML(saveDir);
    if (siteHTML.open(QFile::WriteOnly))
    {
        QTextStream out(&siteHTML);
        out << m_site->getPage(0)->toString().c_str();
    }

    qDebug()<<saveDir;
    //QDesktopServices::openUrl(QUrl("file://"+saveDir));//"https://openclassrooms.com/forum/sujet/qt-ouvrir-une-page-web-a-l-aide-d-un-qpushbutton-39280"));
}
...
void MainWindow::nouveau()
{
    ...
    projectName = QInputDialog::getText(this,"Nouveau","Entrez le nom du nouveau projet", QLineEdit::Normal, QString(), &ok);
...
void MainWindow::ouvrir()
{
    ...
    std::cout<<"soka="<<"soso"<<std::endl;
    QString fichier = QFileDialog::getOpenFileName(this);
    std::cout<<"soka="<<fichier.toStdString();
    while(fichier == "")
    {
